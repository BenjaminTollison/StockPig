...
def add_current_predictions(
    ...
    for game in games.itertuples(index=False):
        ...
        home_X = pd.DataFrame([home_features], columns=FEATURE_COLUMNS)
        away_X = pd.DataFrame([away_features], columns=FEATURE_COLUMNS)

        home_prediction = max(0.0, float(model.predict(home_X)[0]))
        away_prediction = max(0.0, float(model.predict(away_X)[0]))

        output_rows.append(
            {
                **game._asdict(),
                "expected_home_score": home_prediction,
                "expected_away_score": away_prediction,
...
# =============================================================================
# Empirical-residual Monte Carlo
# =============================================================================
...
def simulate_game_from_residuals(
    ...
    """Simulate scores by resampling held-out prediction errors."""
    ...
    home_scores = np.rint(expected_home + home_error)
    away_scores = np.rint(expected_away + away_error)

    home_scores = np.clip(home_scores, 0, None).astype(int)
    away_scores = np.clip(away_scores, 0, None).astype(int)

    return home_scores, away_scores


def score_distribution(scores: np.ndarray) -> pd.DataFrame:
    values, counts = np.unique(scores, return_counts=True)
    probabilities = counts / len(scores)
    return pd.DataFrame({"Score": values, "Probability": probabilities})


def likely_final_scores(home: np.ndarray, away: np.ndarray, top_n: int = 12) -> pd.DataFrame:
    outcomes = pd.DataFrame({"Away": away, "Home": home})
    result = (
...
def game_probabilities(home: np.ndarray, away: np.ndarray) -> tuple[float, float, float]:
    home_win = float(np.mean(home > away))
    away_win = float(np.mean(away > home))
    tie = float(np.mean(home == away))
...
for game in predictions.itertuples(index=False):
    home_sim, away_sim = simulate_game_from_residuals(
        expected_home=float(game.expected_home_score),
        expected_away=float(game.expected_away_score),
        bundle=bundle,
        n=10_000,
        seed=RANDOM_SEED + int(game.game_id) % 100_000,
    )
    home_win, away_win, tie = game_probabilities(home_sim, away_sim)
...
with left:
    ...
    st.write(
        "80% simulated range:",
        f"{np.percentile(away_scores, 10):.0f}–{np.percentile(away_scores, 90):.0f}",
    )
...
with right:
    ...
    st.write(
        "80% simulated range:",
        f"{np.percentile(home_scores, 10):.0f}–{np.percentile(home_scores, 90):.0f}",
    )
