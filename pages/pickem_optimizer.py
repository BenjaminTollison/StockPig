...
import predictor_core as predictor
...
PICKEM_MONTE_CARLO_SIMS = 10_000
...
def build_weekly_win_probabilities(
    ...
    """Create one win-probability row for every SEC team/game in Weeks start..13.

    Team features are frozen at the beginning of planning_start_week. This means
    the optimizer does not pretend to know statistics from future games.
    """
    ...
    for week_number, week in enumerate(weeks, start=1):
        games = predictor.get_sec_week_games(
            schedule,
            int(season),
            int(week),
        )
        ...
        for game in games.itertuples(index=False):
            # A team cannot be selected after its game is already final.
            ...
            # IMPORTANT: features are frozen "as of" the planning start week,
            # rather than using a different future cutoff for every target week.
            ...
            expected_home = max(
                0.0,
                float(bundle.model.predict(home_X)[0]),
            )
            expected_away = max(
                0.0,
                float(bundle.model.predict(away_X)[0]),
            )

            home_scores, away_scores = predictor.simulate_game_from_residuals(
                ...
                seed=(
                    ...
                    + int(game.game_id) % 100_000
                    + int(week) * 100_003
                ),
            )

            home_win, away_win, tie_prob = predictor.game_probabilities(
                home_scores,
                away_scores,
            )

            # College football games do not end in ties. Our integer-score
            # simulation can produce equal scores, so split that artifact evenly.
            home_probability = float(home_win + 0.5 * tie_prob)
            away_probability = float(away_win + 0.5 * tie_prob)
                ...
                rows.append(
                    {
                        ...
                        "expected_points": expected_home,
                        "opponent_expected_points": expected_away,
                    }
                )
                ...
                rows.append(
                    {
                        ...
                        "expected_points": expected_away,
                        "opponent_expected_points": expected_home,
                    }
                )

        if progress_callback is not None:
            progress_callback(
                int(100 * week_number / total_weeks),
                (
                    f"Generating weekly win probabilities: "
                    f"Week {week} ({week_number}/{total_weeks})"
                ),
            )
